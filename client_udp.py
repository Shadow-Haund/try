...
def receive_message():
    ...
    while True:
        # if buffer == 0:
        #     message_big_buffer, address = client.recvfrom(lenght)
        #     message = message_big_buffer
        #     if message.strip().isdigit():
        #         buffer = int(message.strip())
        #     if message.strip() == f"file{file_end}".encode(code_table):
        #         # file_header_len, address = int(client.recvfrom(lenght).decode())
        #         file_name_size, address = client.recvfrom(lenght)
        ...
        #         file_data_write, address = client.recvfrom(lenght)
